...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        default_gyro_file = exp_path / "IMU" / "Sensor_3" / "gyro_010_Waiting_for_static_turns.csv"
    ...
    if not os.path.exists(gyro_file):
        ...
        # Try to find the file in other locations using path helper
        alt_experiments = [
            ("010_Waiting_for_static_turns", "gyro_010_Waiting_for_static_turns.csv"),
            ("011_Static_stbd_1", "gyro_011_Static_stbd_1.csv"),
            ("012_Static_port_1", "gyro_012_Static_port_1.csv")
        ]
        
        for exp_name, gyro_filename in alt_experiments:
            exp_path = get_experiment_path(exp_name, "afternoon")
            full_path = exp_path / "IMU" / "Sensor_3" / gyro_filename
            if full_path.exists():
                print(f"\nFound alternative file: {exp_name}")
                analyze_gyro_data(str(full_path))
                return
